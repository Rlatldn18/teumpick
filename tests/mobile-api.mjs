...
const details={name:'검증 매장',category:'한식',station:'신도림',address:'서울 테스트로 123',description:'테스트',minutes:15,open:true};
const menus=[{id:'main',name:'테스트 덮밥',group:'메인 메뉴',description:'따뜻한 덮밥',price:9500,available:true},{id:'drink',name:'아이스티',group:'음료',description:'복숭아',price:2500,available:true},{id:'dessert',name:'쿠키',group:'디저트',description:'품절',price:1000,available:false}];
await request('merchant',{token:seller.token,body:{...details,menus}});
const saved=await request('merchant',{token:seller.token});assert.deepEqual(saved.menus,menus);
await request('merchant',{token:buyer.token,body:{...details,menus},status:403});
await request('merchant',{token:seller.token,body:{...details,menus:[menus[0],menus[0]]},status:400});
await request('merchant',{token:seller.token,body:{...details,menus:menus.map(m=>({...m,available:false}))},status:400});passed++;
const drinkOrder={action:'create',shopId:merchant.id,menuId:'drink',unitPrice:2500,station:'신도림',qty:2,note:'',requestId:crypto.randomUUID()};
...
await request('orders',{token:seller.token,body:{action:'cancel',id:drink.id}});
const canceled=(await request('orders',{token:otherBuyer.token}))[0];assert.equal(canceled.status,5);assert.equal(canceled.canceledBy,'seller');passed++;

const catalog=await request('catalog');assert.ok(catalog.some(s=>s.id===merchant.id));passed++;
const make=qty=>({action:'create',shopId:merchant.id,menuId:'main',unitPrice:9500,station:'신도림',qty,note:'통합 테스트',requestId:crypto.randomUUID()});
...
assert.ok((await request('orders',{token:otherBuyer.token})).every(o=>o.id!==order.id));
...
await request('orders',{token:otherBuyer.token,body:make(1)});const second=(await request('orders',{token:otherBuyer.token})).find(o=>o.status===0);assert.notEqual(second.locker,order.locker);passed++;
