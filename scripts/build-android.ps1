...
Copy-Item -LiteralPath (Join-Path $root 'android/app/build/outputs/apk/debug/app-debug.apk') -Destination (Join-Path $root 'outputs/android/teumpick-0.4.0-debug.apk')
Copy-Item -LiteralPath (Join-Path $root 'android/app/build/outputs/bundle/release/app-release.aab') -Destination (Join-Path $root 'outputs/android/teumpick-0.4.0-unsigned.aab')
